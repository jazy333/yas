...
namespace yas {
TermQuery::TermQuery(Term* term, IndexStat* index_stat, TermStat* term_stat)
    : term_(term), index_stat_(index_stat), term_stat_(term_stat) {}
...
std::unique_ptr<Matcher> TermQuery::matcher(SubIndexReader* sub_reader) {
  ...
  return std::unique_ptr<Matcher>(
      new TermMatcher(term_reader, new BM25Relevance(), index_stat_, term_stat_));
}
}  // namespace yas