...
namespace yas {
FieldValuesReader::FieldValuesReader(File* meta, File* data)
    : field_values_meta_(meta), field_values_data_(data) {}

FieldValuesReader::~FieldValuesReader() {}

void FieldValuesReader::read() {
  while (true) {
    int field_id;
    meta->read(&field_id, sizeof(field_id));
    if (field_id == -1) {
      break;
    }
    int8_t type;
    std::string field_name;
    meta->read(&type, sizeof(type)) switch (type) {
      case 0: {
...
void FieldValuesReader::read_numeric(const std::string& field_name) {
...
void FieldValuesReader::read_binary(const std::string& field_name) {
...
FieldIndexReader* FieldValuesReader::get(const std::string& field_name) {
