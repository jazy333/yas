...
class Points : public IntroSorter {
 ...
 public:
  ...
  virtual int max_length() { return value_type::bytes_per_dim; }
  ...
  int get_split_dimension(value_type& min, value_type& max,
    ...
    for (int i = 0; i < dim; ++i) {
      if (parent_splits[i] < max_splits / 2 && min.get(i) != max.get(i))
        return i;
    }

    value_type diff = max - min;
    int split_dim = 0;
    T max_dim_diff = diff.get(split_dim);
    for (int i = 1; i < dim; ++i) {
  ...
  void common_prefix_lengths(int from, int to,
  ...
  iterator begin() { return points_iterator(this, 0); }
 ...
 private:
  class points_iterator
   ...
   public:
    ...
    inline points_iterator& operator=(const points_iterator& rhs) {
      _cur = rhs._cur;
      return *this;
    }
    inline points_iterator& operator+=(difference_type rhs) {
      _cur += rhs;
      return *this;
    }
    inline points_iterator& operator-=(difference_type rhs) {
    ...
    inline points_iterator& operator++() {
      ++_cur;
      return *this;
    }
    inline points_iterator& operator--() {
    ...
    // inline points_iterator operator++(int) const { points_iterator
    // tmp(*this); ++_cur; return tmp; } inline points_iterator operator--(int)
    // const { points_iterator tmp(*this); --_cur; return tmp; }
    inline difference_type operator-(const points_iterator& rhs) const {
      return points_iterator(_cur - rhs.cur);
    }
    inline points_iterator operator+(difference_type rhs) const {
      return points_iterator(_cur + rhs);
    }
    inline points_iterator operator-(difference_type rhs) const {
      return points_iterator(_cur - rhs);
    }
    friend inline points_iterator operator+(difference_type lhs,
                                            const points_iterator& rhs) {
      return points_iterator(lhs + rhs._cur);
    }
    friend inline points_iterator operator-(difference_type lhs,
                                            const points_iterator& rhs) {
      return points_iterator(lhs - rhs._cur);
    ...
    inline bool operator==(const points_iterator& rhs) const {
