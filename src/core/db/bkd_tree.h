...
namespace yas {
    ...
    public:
        ...
        int build(bool is_left, int leaves_offset, int num_leaves,long min_block_fp int from, int to, value_type& min, value_type& max,
            ...
            if (num_leaves == 1) {
                leaf_block_fps[leaves_offset]=_kdd->size();
                int count = to - from;
                ...
                write_block_values(from,to,common_prefix_lengths,leaf_cardinality,sorted_dim);
                 ...
                 //generate index
                if(is_left){
                    //do nothing ,the delta is zero
                }else{
                    long delta=leaf_block_fps[leaves_offset]-min_block_fp;
                    //TODO:dump 
                    _kdi->write_vint(delta);
                }
            }else {
                ...
                */
                ...
                if (split_suffix > 1) {
                    ...
                    uchar* svb=split_value.get_bytes_one_dim(split_dim);
    ...
    private:
        ...
        void write_common_prefix(std::vector<int>& common_prefix_lengths, value_type& v) {
            for(int i=0;i<value_type::dim;++i){        
                _kdd->write_vint( common_prefix_lengths[i]);
                u_char* byte s =v.get_bytes();  
                int index=(i+1)*value_type::bytes_per_dim-common_prefix_lengths[i];
        ...
        void write_actual_bounds(int from, int to, const vector<int>& common_prefix_lengths) {
            ...
            for (int dim = 0; dim < value_type::dim; ++dim) {
                ...
                if (suffix_length > 0) {
                    ...
                    _kdd->append(min.get_bytes() + index, suffix_length);
                    _kdd->append(max.get_bytes() + index, suffix_length);
                }
            }
        }
        ...
        void write_high_cardinality_block_values(int from, int to, std::vector<int>& common_prefix_lengths, 
            ...
            for (int i= from; i < to; ) {
                ...
                value_type current = _storage->get(i);
                _kdd->append(current->get_byte(sorted_dim, common_prefix_length[sorted_dim]));
                _kdd->append(char(run), 1);

                for (int j = i;j < i + run;j++) {
                    for (int k = 0;k < value_type::dim;++k) {
                        _kdd->append(k * value_type::bytes_per_dim, value_type::bytes_per_dim - common_prefix_lengths[dim]);
                    }
                }
                i += run;
            }
        ...
        void write_low_cardinality_block_values(int from, int to, std::vector<int>& common_prefix_lengths) {
            ...
            for (int i = from; i < to; i++) {
                ...
                if (value != target || i == to - 1) {
                    _kdd->write_vint(cardinality);
                    for (int j = 0;j < value_type::dim;++j) {
                        _kdd->append(j * value_type::bytes_per_dim, value_type::bytes_per_dim - common_prefix_lengths[dim]);
                    }
                    cardinality = 1;
                    target = value;
                }
        ...
        void write_block_values(int from,int to,std::vector<int>& common_prefix_lengths,int leaf_cardinality,int sorted_dim){
            ...
            if(sum==value_type::bytes_length){
                ...
                if(count==leaf_cardinality){
                    ...
                    for (int i = 0; i < count; ) {
                    // do run-length compression
                        int run= run_len(from,to,sorted_dim);
                        num_run_lens++;
                        i += run;
                    }
                ...
                if(low_cardinality_cost<=high_cardinality_cost){
                    _kdd->append(char(-2),1);
                    write_low_cardinality_block_values(from,to,common_prefix_lengths);
                }else{
                    _kdd->append(char(sorted_dim),1);
                    write_high_cardinality_block_values(from,to,common_prefix_lengths,sorted_dim);
                }
            }
        }
        ...
        std::vector<std::shared_ptr<MemoryFile>> _nodes;
        File* _kdd;
        File* _kdi;
        File* _kdm;
    };
